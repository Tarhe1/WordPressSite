 ...
 * is migrated to Pressable from another host
 *********************************************************
 */

$pressable_site_id_db = 'pressable_site_id';
...
//     $pressable_site_id_option = $options['pressable_site_id'];

//     global $pagenow;

//     if ($pagenow == 'admin.php' && $pressable_site_id_option !== get_option($pressable_site_id_db))
//     {
//         return;

//     }
//     elseif ($pressable_site_id_option !== get_option($pressable_site_id_db))
//     {
...
// }
// $option_name = get_option('pressable_site_id');

// if ('not-exists' === get_option('pressable_api_authentication_tab_options', 'not-exists' || get_option($option_name)))
// {
...
$options['pressable_site_id']  = true;
$option_name = get_option('pressable_site_id');

    if ($pagenow == 'admin.php' && $options['pressable_site_id'] !== get_option($option_name))
    {
        return;

    }
    elseif ( $options['pressable_site_id']  = true && $options['pressable_site_id'] !== get_option($option_name))
...

{
    ...
    //Auto enable CDN radio options if authentication is sucessfull
    $radio_option = array(
...
function pressable_cache_management_callback_field_secret_text($args)
{
    ...
    echo '<input autocomplete="off" id="ppressable_api_authentication_tab_options_' . $id . '" name="pressable_api_authentication_tab_options[' . $id . ']" type="text" size="40" value="' . $value . '"><br/>';
    echo '<label class="rad-text for="pressable_api_authentication_tab_options_' . $id . '">' . $label . '</label>';
